...
class JobseekerController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        return view('job-seeker.userdashboard', compact('categories', 'jobs', 'cat', 'search'));
    }
    ...
    public function profile() {
        ...
        $works = Work::where('user_id', $user->id)
                    ->orderBy('created_at', 'desc')
                    ->get(); 
        return view('job-seeker.profile', compact('user', 'profile', 'skills', 'educations', 'works'));
    }
    ...
    public function updateProfile(Request $request) {
     ...
     public function uploadPhoto(Request $request) {
        ...
        /*dd($profile);*/
         $errMsgs = [
             'profilepicture.required' => 'Please Submit Your Photo',
         ];
         $validation_expression = [
             'profilepicture' => 'mimes:jpeg,jpg,png|required',
         ];

         $validator = Validator::make($request->all(), $validation_expression, $errMsgs);
         if(!$validator->fails()):

                 $data['photo'] = ExtraController::savePhoto($request->profilepicture);
             /*dd($data);*/
             if ($profile == null):
                 /*dd("profile nei");*/
                 ...
                 $profile = Profile::find($id)->update($data);
             endif;
    ...
    public function cv(Request $request){
        ...
        /*dd($profile);*/
        $errMsgs = [
            'cv.required' => 'Please Submit Your File',
        ];
        $validation_expression = [
            'cv' => 'mimes:doc,docx,pdf|required',
        ];

        $validator = Validator::make($request->all(), $validation_expression, $errMsgs);
        if(!$validator->fails()):

            $data['cv'] = ExtraController::saveFile($request->cv);
            /*dd($data);*/
            if ($profile == null):
                /*dd("profile nei");*/
                $data['user_id'] = $user->id;
    ...
    public function myJobs() {
        ...
        $jobs = DB::table('applicants')
            ...
            ->get();
        return view('job-seeker.my_jobs')->withUser($user)->withJobs($jobs);
    }
