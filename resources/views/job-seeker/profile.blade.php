...
{{--cv upload--}}
                    ...
                    <div class="card-header">
                        <a class="card-title">
                            <h5 class="d-inline-block h5 text-success font-weight-bold mb-0">Your CV</h5>
                            <button class="btn btn-primary float-right  py-0 mr-1 px-1" data-toggle="modal" data-target="#uploadcv{{$user->id}}">
                                <i class="far fa-edit text-white"></i> <span class="text-white h6">Upload</span>
                            </button>
                        </a>
                    </div>
                    <div>
                        <div class="card-body cvBackgroundBody">
                                <div>
                                  @if(!empty($profile) && isset($profile->cv))
                                      @if($profile->cv==="")
                                      @else
                                        <p class="float-right text-info mr-4">
                                            <i class="fas fa-pencil-alt" data-id="{{$profile->id}}" data-toggle="modal" data-target="#uploadcv{{$user->id}}"></i>
                                        </p>
                                        <h5 class="h5 text-info"><a target="_blank" href="{{ \App\Http\Controllers\ExtraController::photoURL($profile->cv) }}">{{$profile->cv}}</a></h5>
                                        <hr>
                                      @endif
                                    @endif
                                </div>
                    ...
                    <div class="modal fade" id="uploadProject{{$user->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title text-info" id="exampleModalLabel">Add Project</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
                              <div class="modal-body editworksbody">
                                <div class="input-group mb-3">
                                  <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="fas fa-user"></i>&nbsp;Project Name</span>
                                  </div>
                                  <input type="text" id="addProject" class="form-control">
                                </div>
                                <div class="input-group mb-3">
                                  <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="fas fa-user"></i>&nbsp;Project Role</span>
                                  </div>
                                  <input type="text" id="addProjectRole" class="form-control">
                                </div>
                                <div class="input-group mb-3">
                                  <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="far fa-building"></i>&nbsp;Company Name</span>
                                  </div>
                                  <input type="text" id="addProjectCompany" class="form-control">
                                </div>  
                                <div class="input-group mb-3">
                                  <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="far fa-calendar"></i>&nbsp;Duration</span>
                                  </div>
                                  <input type="text" id="addProjectDuration" class="form-control">
                                </div>
                                <div class="form-group">
                                  <span class="input-group-text"><i class="fas fa-briefcase"></i>&nbsp;Description</span>
                                  <textarea class="form-control" id="addProjectDescription" rows="3"></textarea>
                                </div>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                <button type="submit" class="btn btn-primary addProjectButton" data-dismiss="modal">Save changes</button>
                              </div>
                            </div>
                          </div>
                    </div>
                    {{--Project History Starts--}}
                    <div class="card-header">
                        <a class="card-title">
                            <h5 class="d-inline-block h5 text-success font-weight-bold mb-0">Project History</h5>
                            <button class="btn btn-primary float-right  py-0 mr-1 px-1" data-toggle="modal" data-target="#uploadProject{{$user->id}}">
                                <i class="far fa-edit text-white"></i> <span class="text-white h6">Add New</span>
                            </button>
                        </a>
                    </div>
                    <div>
                        <div class="card-body ProjectBodyBaground">
                                @php($projects=\App\Project::where('user_id',$user->id)->get());
                            @foreach($projects as $project)
                            <div>
                                <p class="float-right text-danger targetWorkDiv">
                                    <i class="far fa-trash-alt" data-toggle="modal" data-target="#deleteProject{{$project->id}}"></i>
                                </p>
                                <p class="float-right text-info mr-4">
                                    <i class="fas fa-pencil-alt" data-id="{{$project->id}}" data-toggle="modal" data-target="#editProject{{$project->id}}"></i>
                                </p>
                                <h5 class="h5 text-info">{{$project->name}}</h5>
                                <h6 class="h6 text-black">{{$project->company_name}}</h6>
                                <small class="h6 mb-2 text-muted">{{ $project->duration }}</small>
                                <div class="mt-3 text-muted">{!! nl2br(e($project->description)) !!}</div>
                                <hr>
                            </div>
                         ...
                         <div class="modal fade" id="editProject{{$project->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg" role="document">
                                  <div class="modal-content">
                                    <div class="modal-header">
                                      <h5 class="modal-title text-info" id="exampleModalLabel">Edit Project</h5>
                                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                      </button>
                                    </div>
                                    <div class="modal-body editProjectsbody">
                                      <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <span class="input-group-text"><i class="fas fa-user"></i>&nbsp;Project Name</span>
                                        </div>
                                        <input type="text" id="editProjectName" class="form-control" value="{{$project->name}}">
                                      </div>
                                      <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <span class="input-group-text"><i class="far fa-building"></i>&nbsp;Project Role</span>
                                        </div>
                                        <input type="text" id="editProjectRole" class="form-control" value="{{$project->role}}">
                                      </div>
                                      <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <span class="input-group-text"><i class="far fa-calendar"></i>&nbsp;Company Name</span>
                                        </div>
                                        <input type="text" id="editProjectCompanyName" class="form-control" value="{{$project->company_name}}">
                                      </div>
                                      <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                          <span class="input-group-text"><i class="far fa-calendar"></i>&nbsp;Duration</span>
                                        </div>
                                        <input type="text" id="editProjectDuration" class="form-control" value="{{$project->duration}}">
                                      </div>
                                      <div class="form-group">
                                        <span class="input-group-text"><i class="fas fa-briefcase"></i>&nbsp;Description</span>
                                    ...
                                    <div class="modal-footer">
                                      <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                      <button type="submit" class="btn btn-primary editProject" data-dismiss="modal" data-id="{{$project->id}}">Save changes</button>
                                    </div>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                      <h6 class="modal-title h6">Are you sure you want to delete <span class="text-info">"{{$project->name}}"</span> from your profile?</h6>
                                    </div>
                                    <!-- Modal footer -->
                                    <div class="modal-footer">
                                      <button type="button" class="btn btn-danger text-white px-5" data-dismiss="modal">No</button>
                                      <button type="button" class="btn btn-primary deleteProject px-5" data-dismiss="modal" data-id="{{$project->id}}" >Yes</button>
                                    </div>
                    ...
                    {{--add cv model--}}
                    <div class="modal fade" id="uploadcv{{$user->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <form action="{{route('profile.cv')}}" method="POST" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <div class="modal-header">
                                        <h5 class="modal-title text-info" id="exampleModalLabel">Upload Cv</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body editworksbody">
                                        <div class="form-group">
                                            <input type="file" class="form-control-file text-center" id="cv" name="cv" aria-describedby="fileHelp">
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
